...
<div class="table-responsive col-lg-9">
    ...
    <table class="table table-striped table-sm" id="tbl">
      ...
      <tbody id="tbody">
        @foreach ($treatmentunits as $treatmentunit)
          <tr>
              <td>{{ $loop->iteration }}</td>
              <td>{{ $treatmentunit->name }}</td>
              <td>
                <button class="badge bg-warning border-0" id="editbutton" data-bs-toggle="modal" data-bs-target="#editmodal"><a href="{{ $treatmentunit->id }}"></a><i class="fa fa-edit"></i></button>
                <form action="/treatmentunits/{{ $treatmentunit->id }}" method="post" class="d-inline">
                  @method('delete')
                  @csrf
