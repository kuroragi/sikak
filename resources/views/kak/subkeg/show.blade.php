...
<div class="table-responsive col-lg-9">
    <button class="btn btn-block btn-primary mb-3" id="addbutton" data-bs-toggle="modal" data-bs-target="#addkakmodal"><a href="{{ $subkeg->id }}"></a><i class="fa fa-plus"></i> Tambah KAK</button> <a href="/skpdprogsubkeg?id_kegprog={{ $subkeg->kegprog_id }}"><button class="btn btn-block btn-warning border-0 mb-3"><i class="fa fa-angles-left"></i> Kembali</button></a>
      ...
      <tbody id="tbody">
        @foreach ($subkeg->kak as $kak)
          <tr>
              ...
              <td>@if($kak->sub_unit != ''){{ $kak->subunit->name }}@endif</td>
              <td>
                @php 
                  $total = 0;
                  foreach($kak->tahap as $th){
                    foreach($th->aktivitas as $akt){
      ...
      <tbody id="tbody">
        @foreach ($subkeg->kakgaji as $kakg)
          <tr>
              ...
              <td>@if($kakg->sub_unit != ''){{ $kakg->subunit->name }}@endif</td>
              <td>Rp.
                @php 
                  $total = 0;
                    foreach($kakg->kebutuhanaktgaji as $keb){
                      $total += $keb->jumlah;
                    }
                  echo number_format($total);
                @endphp
      ...
      <tbody id="tbody">
        @foreach ($subkeg->kakrutin as $kakr)
          <tr>
              ...
              <td>@if($kakr->sub_unit != ''){{ $kakr->subunit->name }}@endif</td>
              <td>Rp. 
                @php 
                  $total = 0;
                    foreach($kakr->kebutuhanaktrutin as $keb){
                      $total += $keb->total;
                    }
                  echo number_format($total);
                @endphp
...
<!-- Modal Add KAK -->
<div class="modal fade" id="addkakmodal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <form method="post" id="addkakform" action="/kak" class="mb-5">
          ...
          <div class="row">
              <div class="col col-xl-12">
                  <input type="text" name="subkeg_id" class="form-control @error('subkeg_id') is-invalid @enderror" id="addsubkeg_id" value="{{ old('subkeg_id') }}" hidden>
                  <div class="mb-3">
                    <label for="kelompokbelanja_slug" class="form-label">Kelompok Belanja</label>
                    <select name="kelompokbelanja_slug" class="form-control" id="addkelompokbelanja_slug">
                  ...
                  <div class="row mb-3">
                    ...
                    <div class="row mb-3">
                      <div class="col-md-12 mb-1">
                        <label for="name" class="form-label" id="addisubkeg">Indikator Subkegiatan</label>
                      </div>
                  ...
                  <div class="mb-3" id="kakform">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="addkakname" value="{{ old('name') }}" autofocus>
                        @error('name')
                            <div class="invalid-feedback">
                              {{ $message }}
                            </div>
                    ...
                    <div class="mb-3">
                      ...
                      <input type="text" name="bidang_sekretariat" class="form-control @error('bidang_sekretariat') is-invalid @enderror" id="addbidang_sekretariat" value="{{ old('bidang_sekretariat') }}">
...
<script>
      ...
      $("#addkakbutton").click(function(){
        var id = $(this).find("a").attr("href");
        var ind = $(this).find("a1").attr("href");
        var sat = $(this).find("a2").attr("href");
        $("#addsubkeg_id").val(id);
        $("#addisubkeg").text(ind);
        $("#satsubkeg").text(sat);
      });

      $("#addkelompokbelanja_slug").change(function(){
        var kebe = $("#addkelompokbelanja_slug").val();
        var pekebe = $("#addpencetuskebe_id");
        var nkak = $("#kakform");
        console.log(kebe);

        if(kebe == 'gaji-sttp'){
          $("#addkakform").attr("action","/kakgaji")
          ...
          $("#addsampai").val("");
          $("#adddeskrip").val("");
          $("#addbidang_sekretariat").val("");
          $("#addsubbagdkk").val("");
        }else if(kebe == 'rutin-kantor-wajib'){
          $("#addkakform").attr("action","/kakrutin")
          nkak.css("display", "none");
          $("#tarsubkeg").val("");
          $("#addihaskeg").val("");
          $("#addvolhaskeg").val("");
