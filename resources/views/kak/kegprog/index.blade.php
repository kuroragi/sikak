...
<div class="table-responsive">
  <button class="btn btn-block btn-primary mb-3" id="addbutton" data-bs-toggle="modal" data-bs-target="#addmodal"><i class="fa fa-plus"></i> Tambah Kegiatan</button>
    ...
    <tbody id="tbody">
      @foreach ($kegprog as $k)
        <tr class="align-middle">
            ...
            <td>{{ $k->kode_progbid }}</td>
            <td>{{ $k->kode }}</td>
            <td>{{ $k->progbid->ket??'' }}</td>
            <td>{{ $k->ket }}</td>
            <td class="text-center">@if ($k->status == 1) <i class="text-green fa fa-square"></i> @else <i class="text-red fa fa-square"></i> @endif</td>
            <td class="text-center">
              <button class="btn btn-warning btn-sm" title="Edit Urusan" id="editbutton" data-bs-toggle="modal" data-bs-target="#editmodal"><a kode="{{ $k->kode }}"></a><i class="fa fa-edit"></i></button>
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" action="/kegprog" class="mb-5">
          @csrf
          <div class="row">
              <div class="col col-xl-12">
                  <div class="mb-3">
                      <label for="kode_progbid" class="form-label">Program</label>
                      <select name="kode_progbid" class="form-select" id="addkode_progbid">
                        @foreach ($progbid as $k)
      ...
      <div class="modal-footer">
        <button type="submit" class="btn btn-success"><i class="fa fa-floppy-disk"></i> Tambah Kegiatan</button>
      </form>
...
<div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
  <div class="modal-content">
      ...
      <form method="post" id="editform" action="/supliers" class="mb-5">
        ...
        <div class="row">
          <div class="col col-xl-12">
              <div class="mb-3">
                  <label for="kode_progbid" class="form-label">Program</label>
                  <select name="kode_progbid" class="form-select" id="editkode_progbid">
                    @foreach ($progbid as $p)
                          <option value="{{ $p->kode }}">[{{ $p->kode }}] {{ $p->ket }}</option>
                    @endforeach
                  </select>
              </div>
              <div class="mb-3">
    ...
    <div class="modal-footer">
      <button type="submit" class="btn btn-warning"><i class="fa fa-edit"></i> Ubah Kegiatan</button>
    </form>
      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
    </div>
  </div>
...
<script>
    ...
    $("#tbody #editbutton").click(function(){
      var kode = $(this).find("a").attr("kode");
        $.ajax({
        url: '/getdataedit?tbl=kegprogs&whr=kode&id='+kode,
        type: 'GET',
        success: function(data){
        console.log(data);
          $("#editform").attr("action", "/kegprog/"+kode);
          $("#editkode_progbid").val(data.kode_progbid).change();
