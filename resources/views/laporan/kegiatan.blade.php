...
<div class="table-responsive col-lg-12">
  <a href="/laporan_program?tipe={{ request('tipe') }}&kode_skpd={{ request('kode_skpd') }}&periode={{ request('periode') }}"><button class="btn btn-block btn-warning"><i class="fa fa-angles-left"></i> Kembali</button></button></a>
  <button class="btn btn-block btn-primary" id="printout" data-bs-toggle="modal" data-bs-target="#printoutmodal"><i class="fa fa-print"></i> Print Semua Laporan {{ strtoupper(request('tipe')) }} {{ $progbid->ket }}</button>  
      ...
      <tbody id="tbody">
        @foreach ($kegprog as $kp)
          <tr class="text-align-center">
              ...
              <td class="text-center"><a href="/laporan_subkeg?tipe={{ request('tipe') }}&kode_skpd={{ request('kode_skpd') }}&kode={{ $kp->kode }}&periode={{ request('periode') }}"><button class="badge bg-info border-0" title="Pilih Kegiatan"><i class="fa fa-eye"></i> Lihat Subkegiatan</button></a></td>
          </tr>
        @endforeach
      </tbody>
    </table>
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" action="/cetaklaporan?tipe={{ request('tipe') }}&getlaporan=progbid&kode_skpd={{ request('kode_skpd') }}&kode={{ $progbid->kode }}&periode={{ request('periode') }}" class="mb-5">
          @csrf
          <div class="row">
