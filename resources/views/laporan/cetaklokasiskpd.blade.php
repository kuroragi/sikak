    ...
    <body>
        ...
        @if($jenis == 'pdf')
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h2 class="text-center">Pagu SKPD</h2>
                    <h2 class="text-center">Se-{{ ucwords($lokasi->ket) }}</h2>
                </div>
                ...
                <div class="table-responsive col-lg-12 mt-3">
                    ...
                    <thead>
                        <tr class="text-center fs-8 text-wrap">
                            <th scope="col">No</th>
                            <th scope="col" class="text-center">SKPD</th>
                            @foreach ($kebe as $kb)
                                <th scope="col">{{ $kb->ket }}</th>
                            @endforeach
                        ...
                        @foreach ($skpd as $s)
                            <tr class="fs-8">
                                ...
                                @foreach ($kebe as $kb)
                                    ...
                                    @endforeach
                                    @php
                                        $tkel['total'.$kb->id] += $totallok;
                                        $totalskpd += $totallok;
                                        $totalallpskpd += $totallok;
                                    @endphp
                                    <td class="text-end">
                                        {{ str_replace(',', '.', number_format($totallok)) }}
                                    </td>
                                @endforeach
                                <td class="text-end">{{ str_replace(',', '.', number_format($totalskpd)) }}</td>
                            </tr>
                        
                        @endforeach
                        <tr class="fs-8 tbold">
                            <td class="text-wrap" colspan="2">Total Kelompok belanja</td>
                            @foreach ($kebe as $kb)
                                <th scope="col" class="text-end">{{ str_replace(',', '.', number_format($tkel['total'.$kb->id])) }}</th>
                                @php
                                    $totalall += $tkel['total'.$kb->id];
                                @endphp
                            @endforeach
                            <td class="text-end">{{ str_replace(',', '.', number_format($totalall)) }}/{{ str_replace(',', '.', number_format($totalallpskpd)) }}</td>
                        </tr>
                    </tbody>
                    </table>
