...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Instrumen Dalam Kegiatan</h1>
    <div class="btn-toolbar mb-2 mb-md-0">
    </div>
</div>

<div class="table-responsive col-lg-9">
    <button class="btn btn-block btn-primary mb-3" id="addbutton" data-bs-toggle="modal" data-bs-target="#addmodal"><i class="fa fa-plus"></i> Tambah Instrumen</button>
      ...
      <tbody id="tbody">
        @foreach ($instru as $i)
          <tr>
              ...
              <td>{{ $i->name }}</td>
              <td>
                <button class="badge bg-warning border-0" id="editbutton" data-bs-toggle="modal" data-bs-target="#editmodal"><a href="{{ $i->slug }}"></a><i class="fa fa-edit"></i></button>
                <form action="/instrumen/" method="post" class="d-inline">
                  @method('delete')
                  @csrf
  ...
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Tambah Instrumen</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form method="post" action="/instrumen" class="mb-5">
          @csrf
          <div class="row">
      ...
      <div class="modal-footer">
        <button type="submit" class="btn btn-primary">Tambah Instrumen</button>
      </form>
...
<div class="modal fade" id="editmodal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" id="editform" action="/instrumen" class="mb-5">
          @csrf
          @method('put')
          <div class="row">
              <div class="col col-xl-12">
      ...
      <div class="modal-footer">
        <button type="submit" class="btn btn-primary">Edit Instrumen</button>
      </form>
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
      </div>
    </div>
...
<script>
      ...
      $("#tbody #editbutton").click(function(){
        var id = $(this).find("a").attr("href");
          $.ajax({
          url: 'getdataedit?id='+id+'&tbl=instrumens&whr=slug',
          type: 'GET',
          success: function(data){
            $("#editform").attr("action", "/instrumen/"+id);
