      ...
      <tbody id="tbody">
        @foreach ($kelurahan as $k)
          <tr class="text-center">
              ...
              <td class="text-start">{{ $k->kecamatan->ket }}</td>
  ...
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" action="/kelurahan" class="mb-5">
          @csrf
          <div class="row">
              <div class="col col-xl-12">
                <div class="mb-3">
                    <label for="ket" class="form-label">Kelurahan</label>
                ...
                <div class="mb-3">
                  ...
                  <select name="kecamatan_id" class="form-select" id="addkecamatan_id">
                    @foreach ($kecamatan as $kec)
                        <option value="{{ $kec->id }}">{{ $kec->ket }}</option>
                    @endforeach
                  </select>
                </div>
                <div class="mb-3">
...
<div class="modal fade" id="editmodal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="post" id="editform" action="/kelurahan" class="mb-5">
            @csrf
            @method('put')
            <div class="row">
                <div class="col col-xl-12">
                  ...
                  <div class="mb-3">
                    <label for="status" class="form-label">Kecamatan</label>
                    <select name="kecamatan_id" class="form-select" id="editkecamatan_id">
                      @foreach ($kecamatan as $kec)
                          <option value="{{ $kec->id }}">{{ $kec->ket }}</option>
                      @endforeach
                    </select>
...
<script>
      ...
      $("#tbody #editbutton").click(function(){
        var id = $(this).find("a").attr("id");
          $.ajax({
          url: 'getdataedit?&tbl=kelurahans&whr=id&id='+id,
          type: 'GET',
          success: function(data){
            $("#editform").attr("action", "/kelurahan/"+id);
            $("#editket").val(data.ket);
            $("#editkecamatan_id").val(data.kecamatan_id).change();
